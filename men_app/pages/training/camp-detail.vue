  ...
  <view class="camp-detail-container">
    ...
    <view class="header">
      ...
      <view class="camp-info">
        ...
        <text class="camp-name">军事训练营</text>
    ...
    <block v-if="campType === 'rookie'">
      ...
      <view class="training-module">
        ...
        <view class="module-description">完成俯卧撑、卷腹、深蹲各9000个训练，打造军人基本体能素质</view>
          ...
          <view class="exercise-card" @click="startExercise('pushups')">
            ...
            <view class="exercise-content">
              <view class="exercise-header">
                ...
                <view class="exercise-status" :class="trainingData.pushups >= 9000 ? 'completed' : 'in-progress'">
                  {{ getExerciseStatus('pushups') }}
                </view>
              </view>
              <view class="exercise-progress-container">
                <view class="exercise-count">{{ trainingData.pushups || 0 }}/9000 次</view>
                <view class="exercise-progress-bar">
                  <view class="exercise-progress-fill" :style="{width: Math.min((trainingData.pushups || 0) / 9000 * 100, 100) + '%'}"></view>
                </view>
              </view>
            </view>
          </view>
          ...
          <view class="exercise-card" @click="startExercise('situps')">
            ...
            <view class="exercise-content">
              <view class="exercise-header">
                ...
                <view class="exercise-status" :class="trainingData.situps >= 9000 ? 'completed' : 'in-progress'">
                  {{ getExerciseStatus('situps') }}
                </view>
              </view>
              <view class="exercise-progress-container">
                <view class="exercise-count">{{ trainingData.situps || 0 }}/9000 次</view>
                <view class="exercise-progress-bar">
                  <view class="exercise-progress-fill" :style="{width: Math.min((trainingData.situps || 0) / 9000 * 100, 100) + '%'}"></view>
                </view>
              </view>
            </view>
          </view>
          ...
          <view class="exercise-card" @click="startExercise('squats')">
            ...
            <view class="exercise-content">
              <view class="exercise-header">
                ...
                <view class="exercise-status" :class="trainingData.squats >= 9000 ? 'completed' : 'in-progress'">
                  {{ getExerciseStatus('squats') }}
                </view>
              </view>
              <view class="exercise-progress-container">
                <view class="exercise-count">{{ trainingData.squats || 0 }}/9000 次</view>
                <view class="exercise-progress-bar">
                  <view class="exercise-progress-fill" :style="{width: Math.min((trainingData.squats || 0) / 9000 * 100, 100) + '%'}"></view>
                </view>
              </view>
            </view>
          </view>
      ...
      <view class="training-module">
        ...
        <view class="module-description">完成30次3公里跑训练，提升心肺能力和全身耐力</view>
        
        <view class="run-card">
          <view class="run-header">
            <text class="run-title">3公里耐力跑</text>
            <view class="run-status" :class="trainingData.runningCompleted >= 30 ? 'completed' : 'in-progress'">
              {{ getRunningStatus() }}
            </view>
          </view>
          
          <view class="run-progress-container">
            <view class="run-count">{{ trainingData.runningCompleted || 0 }}/30 次</view>
            <view class="run-progress-bar">
              <view class="run-progress-fill" :style="{width: Math.min((trainingData.runningCompleted || 0) / 30 * 100, 100) + '%'}"></view>
            </view>
          </view>
            ...
            <view class="run-stats">
              <view class="run-stat-item">
                ...
                <view class="run-stat-content">
                  ...
                  <text class="run-stat-value">{{ formatRunningTime(trainingData.bestRunningTime || 0) }}</text>
                </view>
              </view>
              ...
              <view class="run-stat-item">
                ...
                <view class="run-stat-content">
                  ...
                  <text class="run-stat-value">{{ formatRunningTime(getRunningTargetTime()) }}</text>
                </view>
              </view>
            </view>
          </view>
      ...
      <view class="training-module">
        ...
        <view class="module-description">完成所有训练项目后参加结业考核，通过考核晋升军衔</view>
          ...
          <view class="exam-info">
            <view class="exam-content" v-if="examStats.completed">
              <view class="exam-result-item">
                ...
                <view class="exam-result-value">{{ formatRunningTime(examStats.examTime) }}</view>
              </view>
              <view class="exam-result-item">
                <view class="exam-result-label">考核评级</view>
                <view class="exam-result-value" :class="{'exam-excellent': getExamGrade() === '优秀', 'exam-pass': getExamGrade() === '合格', 'exam-fail': getExamGrade() === '不合格'}">{{ getExamGrade() }}</view>
              </view>
            </view>
            <view class="exam-status" v-else>
              <view class="exam-status-icon" :class="{'available': canTakeFinalExam}">
                {{ canTakeFinalExam ? '✓' : '✗' }}
              </view>
              <view class="exam-status-text">{{ canTakeFinalExam ? '已满足考核条件' : '需完成所有训练' }}</view>
            </view>
          </view>
          
          <button class="action-button" :class="{'disabled': !canTakeFinalExam && !examStats.completed}" @click="startFinalExam">
            <view class="button-icon">{{ examStats.completed ? '🏆' : '🎯' }}</view>
            <text>{{ examStats.completed ? '查看考核成绩' : '参加结业考核' }}</text>
          </button>
        </view>
      </view>
    </block>
...
export default {
  ...
  methods: {
    // 加载训练营数据
    loadCampData() {
      // TODO: 从服务器获取用户在当前训练营的数据
      // 这里先使用模拟数据，实际开发中应该从API获取
      if (this.campType === "rookie") {
        // 模拟数据已在data中初始化
    ...
    calculateOverallProgress() {
      try {
        if (this.campType === "rookie") {
          ...
          const pushupProgress = Math.min((this.trainingData.pushups || 0) / 9000, 1);
          const situpProgress = Math.min((this.trainingData.situps || 0) / 9000, 1);
          const squatProgress = Math.min((this.trainingData.squats || 0) / 9000, 1);
          
          // 计算3公里跑进度 (30次完成)
          const runningProgress = Math.min((this.trainingData.runningCompleted || 0) / 30, 1);
          ...
          this.overallProgress = Math.floor((basicAvgProgress * 0.6 + runningProgress * 0.3 + examProgress * 0.1) * 100);
        }
      } catch (e) {
    ...
    getExamGrade() {
      ...
      if (examTime <= 15 * 60) { // 15分钟内
        return '优秀';
      } else if (examTime <= 18 * 60) { // 18分钟内
        return '合格';
      } else {
        return '不合格';
      }
    ...
    startFinalExam() {
      ...
      if (this.examStats.completed) {
        uni.showModal({
          title: '考核成绩',
          content: `考核时间: ${this.formatRunningTime(this.examStats.examTime)}\n等级: ${this.getExamGrade()}`,
          showCancel: false
        });
        return;
      }
